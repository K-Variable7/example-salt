...
def main():
    ...
    parser.add_argument(
        "--password", "-p", default="demoPass123!", help="Password to benchmark"
    )
    parser.add_argument("--repeats", "-r", type=int, default=3)
    parser.add_argument("--argon-time", type=int, default=1)
    parser.add_argument(
        "--argon-mem", type=int, default=32768, help="Argon2 memory in KB"
    )
    ...
    if PasswordHasher is not None:
        print(
            f"Benchmarking Argon2: time={args.argon_time}, mem={args.argon_mem}KB, par={args.argon_par}..."
        )
        res = bench_argon(
            args.password,
            time_cost=args.argon_time,
            memory_cost=args.argon_mem,
            parallelism=args.argon_par,
        )
        out["argon2"] = res
        print("  ", res)
    else:
    ...
    # scrypt
    print(
        f"Benchmarking scrypt: N={args.scrypt_n}, r={args.scrypt_r}, p={args.scrypt_p}..."
    )
    try:
        res = bench_scrypt(
            args.password, n=args.scrypt_n, r=args.scrypt_r, p=args.scrypt_p
        )
        out["scrypt"] = res
        print("  ", res)
