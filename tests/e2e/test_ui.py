...
def test_interactive_demo_basic(page):
    ...
    # Dismiss tutorial
    try:
        page.click("#dismissTutorial", timeout=5000)
    except:
        pass
    ...
    page.wait_for_function(
        "() => document.getElementById('demoResults') && document.getElementById('demoResults').innerText.length>0",
        timeout=15000,
    )
...
def test_argon2_happy_path(page):
    ...
    if "did not initialize" in status.lower() or status.lower().startswith(
        "argon2 failed"
    ):
        import pytest
    ...
    assert (
        "Local Argon2" in txt or "Argon2 computed" in status or "argon2" in txt.lower()
    )
